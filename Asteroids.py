...
def process_sprite_group(sprite_group, canvas):
    ...
    for sprite in sprite_group:
        ...
        if sprite.update() == True:
            removal.add(sprite)
    sprite_group.difference_update(removal)
...
# helper function to handle collisions between two groups
def group_group_collide(rocks_group, missiles_group):
    collisions = 0
    for rock in set(rocks_group):
        if group_collide(missiles_group, rock):
            collisions += 1
            rocks_group.discard(rock)
    return collisions
...
def rock_spawner():
    ...
    if started:
        if len(rock_group) < 12:
            ...
            safety_bubble = my_ship.get_radius() * 2
            rock_pos = ship_pos  #force at least one loop
            while dist(rock_pos,ship_pos) < safety_bubble:
